...
# ===============================
# FAISS + BM25 LOAD (from persistent storage)
# ===============================
def load_faiss_bm25(embeddings_file: str, index_file: str):
    """Load FAISS index and embeddings from model directory"""
    path_embeddings = os.path.join(MODEL_DIR, embeddings_file)
    path_index = os.path.join(MODEL_DIR, index_file)
    
    if not os.path.exists(path_embeddings):
        raise FileNotFoundError(f"Embeddings file not found: {path_embeddings}")
    if not os.path.exists(path_index):
        raise FileNotFoundError(f"FAISS index not found: {path_index}")
...
# ===============================
print("📥 Loading FAISS embeddings from disk...")
